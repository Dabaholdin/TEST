...
class Provider extends Model
{
    ...
    public static function getCachedProvidersOrSetProvidersToCache()
    {
        ...
        if (!$cachedProducts) {
            ...
            $providers = unserialize($cachedProducts);
        }

        return ProviderWithRelationshipsResource::collection($providers);
    }

    public static function setProvidersCache(string $cacheKey = self::PROVIDERS_CACHE_KEY)
    {
        $providers = self::withoutTrashed()
                ->with([
                        'catalogs.tags',
                        'country',
                        'sandboxFiles',
        ...
        Redis::set($cacheKey, serialize($providers), 'EX', self::PROVIDERS_CACHE_TTL);
