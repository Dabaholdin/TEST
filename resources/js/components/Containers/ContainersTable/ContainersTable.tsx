...
const ContainersTable: React.FC = () => {
    ...
    const cityFormatter = (city: {name: string}): string | null => {
        return city
            ? city.name
            : null
    }

    const nameFormatter = (name: string, row: {createdAt: number}): string => {
        return `№${name} от ${timeConverter(row.createdAt)}`
    }

    const filterOptions: {label: string, value: any}[] = Object.entries(statuses.containerStatuses)
        .map(([key, value]) => {
            return {
                label: value,
                value: key
