...
const ProductForm: React.FC = () => {
    ...
    // Эта часть мне не нравится. providerId чем должен быть {value: string} или number?
    // Понимаю, что тут хотели поменять переданный formValues, но внутри нет никакой if проверки
    // а она необходима чтобы абстрогироваться от возможной ошибки
    // Добавил оператор ??, это самый минимум что в ней нужно изменить
    const productFormSubmitHandler = handleSubmit((formValues: ICreateProductData) => {
        formValues.image = formValues.image[0]
        formValues.providerId = formValues.providerId?.value ?? formValues.providerId
    ...
    const onChangePrice = (e: React.ChangeEvent<HTMLInputElement>, currencyCode: string): void => {
